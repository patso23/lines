...
class Uploader {
	 ...
	 * @return array
	 */
	public function uploadAll(array $fields = array(), $overwrite = false, $rollback = true, $name = null) {
		if (!$this->_enabled) {
			return false;
		} else {
			$this->checkDirectory();
		...
		if (!empty($fields)) {
			foreach ($fields as $field) {
				if (isset($this->_data[$field])) {
					$upload = $this->upload($field, array('name' => $name, 'overwrite' => $overwrite, 'multiple' => true));
